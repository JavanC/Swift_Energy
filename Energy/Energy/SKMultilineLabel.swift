...
class SKMultilineLabel: SKNode {
    ...
    init(text:String, labelWidth:Int, pos:CGPoint, fontName:String="Verdana-Bold",fontSize:CGFloat=10,fontColor:SKColor=SKColor.blackColor(),leading:Int=10, alignment:SKLabelHorizontalAlignmentMode = .Center, shouldShowBorder:Bool = false)
    {
        self.text = text
...
//                    label.text = "\(lineString) \(words[wordCount])"
            ...
            if lineLength > 0 {
                ...
                self.addChild(label)
                labels.append(label)
//                print("was \(lineLength), now \(label.frame.size.width), words \(words)")
            }
